import {CardStack, Card} from '../common/cardstack';
...
const ShowEmployeeCards = (props) => {
	...
	const cardstackElement = () => (
		<CardStack height={height} width={500} background='#f8f8f8' hoverOffset={25}>
			{(props.employees || []).map(employee => {
					const color = backgroundColors[i % 4];
					i++;
					return (
						<Card background={color} key={employee.firstName + employee.dob} >
      						<EmployeeCard 
	...
	const singleCard = (employee) => (
		<div style={{'background-color': '#2980B9'}}>
      		<EmployeeCard key={employee.firstName + employee.dob}
	      		imgBorderColor='#015389'
	      		employee={employee}
	  		/>
		</div>
		...
		<div className="ui attached segment">
			<div className="ui middle aligned column centered grid">
				...
				<div className="right floated six wide column">
					<button className="ui button" onClick={() => (props.showAddEmployeeForm())}>Add Employee</button>
				</div>
        	</div>
...
const EmployeeCard = ({employee, props = {}}) => (
	<div style={{top: 0 }}>
		<header style={cardStyles.cardHeader} className='card-header-details'>
			<ProfilePicture imgSrc={'/assets/images/' + employee.firstName + '.png'} borderColor={props.imgBorderColor} />
			<div>
